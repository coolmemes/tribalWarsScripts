javascript: (function() {
	! function(t, e) {
		t.Lockr = function(t, e) {
			...
			return e.prefix = "", e._getPrefixedKey = function(t, e) {
				...
				let a = this._getPrefixedKey(t, r);
				try {
				...
				let a, i = this._getPrefixedKey(t, r);
				try {
	...
	function segunda() {
		let html = ['<th> Sending from </th>', '<th> Target Village </th>', '<th> Duration </th>', '<th id="enviar"> > Send < </th>'];
				...
				$.ajax({
					...
					success: function(data) {
						if (!data.error) {
							...
							let html = ['<td style="text-align:center">', minha.c, '</td>', '<td style="text-align:center">', minha.alvoDessa.c, '</td>', '<td style="text-align:center">', tempo, '</td>', '<td id="aindanao" style="text-align:center">Wait...</td>'];
						...
						i++;
						if (i === minhas.length) {
							$('tr[class^="nowrap"]').find('td#aindanao').text("Ready to Send");
							$('#combined_table tbody tr').eq(0).find('th#enviar').text('Send:').append('<input id="Enviar" class="btn" name="x" type="submit" value="Send">');
	...
	function terceira() {
		...
		let delay = 0;
		for (let minha of minhas) {
			let u = ["/game.php?village=", minha.id, "&screen=place&ajaxaction=popup_command&h=", csrf_token, "&client_time=", Math.round(Timing.getCurrentServerTime() / 1e3)];
			setTimeout(() => {
				$.ajax({
					url: u.join(''),
					...
					success: function(data) {
						if (!data.error) {
							$('td#aindanao').eq(i).text("Sent!");
						} else {
							$('td#aindanao').eq(i).text("Error!");
							if (data.error === "A sua sess\u00e3o expirou. Por favor, acesse novamente.") {
								window.location.reload(true);
							}
						}
