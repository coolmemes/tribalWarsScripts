// ==UserScript==
// @name         Premium Exchange - Buy/Sell Resources
// @description  Automatically buy or sell resources
...
const buyOrSell = '<select name="buyOrSell" id="buyOrSell" onchange="selectScript()"> ' +
    '<option value="buy">Buy Resources</option> ' +
    '<option value="sell">Sell Resources</option> ' +
    '</select>';
...
userInputParent.parentNode.insertBefore(buyOrSell, userInputParent);

function selectScript() {
    let chosenScript = document.getElementById("buyOrSell").value;
    document.getElementById("divScript").innerHTML = "";
    if(chosenScript == "buy") {
        buy();
    } else if(chosenScript == "sell") {
        sell();
    }
}
...
function buy() {
     ...
     * @type {Resource}
     */
    let wood = new Resource(game_data.village.wood, parseInt(__("#premium_exchange_rate_wood > div:nth-child(1)").innerText), parseInt(__("#premium_exchange_stock_wood").innerText), 0, 0, __("#premium_exchange_buy_wood > div:nth-child(1) > input"));
    let iron = new Resource(game_data.village.iron, parseInt(__("#premium_exchange_rate_iron > div:nth-child(1)").innerText), parseInt(__("#premium_exchange_stock_iron").innerText), 0, 0, __("#premium_exchange_buy_iron > div:nth-child(1) > input"));
    let stone = new Resource(game_data.village.stone, parseInt(__("#premium_exchange_rate_stone > div:nth-child(1)").innerText), parseInt(__("#premium_exchange_stock_stone").innerText), 0, 0, __("#premium_exchange_buy_stone > div:nth-child(1) > input"));
 ...
 * @constructor
 */
function Resource(wh, price, stock, inc, out, input) {
    this.wh = wh;
    this.price = price;
    this.stock = stock;
    this.inc = inc;
